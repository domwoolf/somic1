...
double doc_leached (double doc, double thickness, double velocity) {
  double leached;
  leached = doc * velocity / thickness; 
  leached = std::min(leached, doc); // we can't leach more than is there
  return (leached);
}
...
List somic(
  ...
  for (unsigned int i = 0; i < n; i++) {
    ...
    if (i > 0) {  // first month has pool sizes and delta 13C values already initialized. Calculate only for subsequent months
      ...
      leached_doc[i] = doc_leached(doc[i], thickness, velocity[i]);
